...
public class ArthurCharacterController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (IsAlive)
        {
            ...
            if (Input.GetMouseButtonDown(1) && CharacterState != State.Attacking)
            {
                HasBeenHit = false;
            }

            if (Input.GetMouseButton(1) || CharacterState == State.Attacking) // Prawy klawisz
            {
                if (Physics.Raycast(ray, out var hit))
                {
